...
function Bonito.jsrender(session::Session, togglers::Togglers)
    ...
    toggles = map(options) do entry
        ...
        modified = DOM.span(
            ...
            onclick=string(js"Bonito.update_obs($(reset), true)")
        )
        onjs(
            session,
            isoriginal,
        ...
        button = DOM.button(
            ...
            onclick=string(js"""
                if (!$(modified).isEqualNode(event.target)) {
                    Bonito.update_obs($selected, !(Bonito.get_observable($selected)))
                }
            """), # FIXME: report to Bonito that this requires string to work on reload
