...
function jsrender(session::Session, wdg::Autocomplete)
    ...
    list = Bonito.jsrender(session, wdg.list)
    ...
    input = DOM.input(
        ...
        spellcheck=false,
        value=wdg.value,
        oninput=js"Bonito.update_obs($(wdg.value), this.value)",
    ...
    onjs(
        ...
        js"""
    function (value) {
        const options = Bonito.get_observable($(wdg.options));
        ...
        Bonito.update_obs($(wdg.list.entries), entries);
    }
"""
    )
